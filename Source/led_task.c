    ...
    while(1)
    {
        ...
        if(xQueueReceive(g_pLEDQueue, &i8Message, 0) == pdPASS)
        {
            ...
            if(i8Message == LEFT_BUTTON)
            {
                ...
                UART_OutString("Led ");
								UART_OutUDec(g_ui8ColorsIndx);
								UART_OutString(" is blinking. [R, G, B]\n");
            ...
            if(i8Message == RIGHT_BUTTON)
            {
                ...
                //
                xSemaphoreTake(g_pUARTSemaphore, portMAX_DELAY);
                UART_OutString("Led blinking frequency is ");
                UART_OutUDec(ui32LEDToggleDelay * 2);
								UART_OutString(" ms.\n");
                xSemaphoreGive(g_pUARTSemaphore);
            }
        }
...
LEDTaskInit(void)
{
    ...
    UART_OutString("\nLed ");
		...
		UART_OutString("Led blinking frequency is ");
		UART_OutUDec(LED_TOGGLE_DELAY * 2);
		UART_OutString(" ms.\n");
